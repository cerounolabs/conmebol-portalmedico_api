...
/*MODULO NOTIFICACION*/
    $app->put('/v2/802/notificacion/{codigo}', function($request) {
        ...
        if (isset($val00) && isset($val00_1)) {
            ...
            switch ($val00_1) {
                case 1:
                    $sql00  = "UPDATE [adm].[NOTFIC] SET NOTFICEST = (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'NOTIFICACIONESTADO' AND DOMFICPAR = ?), NOTFICORD = ?, NOTFICPAC = ?, NOTFICTIT = ?, NOTFICDES = ?, NOTFICFED = ?, NOTFICFEH = ?, NOTFICOBS = ?, NOTFICAUS = ?, NOTFICAFH = GETDATE(), NOTFICAIP = ? WHERE NOTFICCOD = ?";
                    break;
            ...
            try {
                ...
                switch ($val00_1) {
                    case 1:
                        $stmtMSSQL->execute([$val01, $val02, $val05, $val06, $val07, $val08, $val09, $val11, $aud01, $aud03, $val00]);
                    break;
    ...
    $app->put('/v2/802/notificacioncompeticion/{codigo}', function($request) {
        require __DIR__.'/../src/connect.php';
        ...
        if (isset($val00) && isset($val00_1)) {
            ...
            switch ($val00_1) {
                case 1:
                    $sql00  = "UPDATE [adm].[NOTCOM] SET NOTCOMEST = (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'NOTIFICACIONESTADO' AND DOMFICPAR = ?), NOTCOMORD = ?, NOTCOMCOC = ?, NOTCOMNOC = ?, NOTCOMOBS = ?, NOTCOMAUS = ?, NOTCOMAFH = GETDATE(), NOTCOMAIP = ? WHERE NOTCOMCOD = ?";
                    break;
    ...
    $app->put('/v2/802/notificacionequipo/{codigo}', function($request) {
        ...
        if (isset($val00) && isset($val00_1)) {
            ...
            switch ($val00_1) {
                case 1:
                    $sql00  = "UPDATE [adm].[NOTEQU] SET NOTEQUEST = (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'NOTIFICACIONESTADO' AND DOMFICPAR = ?), NOTEQUORD = ?, NOTEQUNCM = ?, NOTEQUEQC = ?, NOTEQUOBS = ?, NOTEQUAUS = ?, NOTEQUAFH = GETDATE(), NOTEQUAIP = ? WHERE NOTEQUCOD = ?";
                    break;
