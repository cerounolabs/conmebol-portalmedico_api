    ...
    $app->get('/v2/200/competicion/home/ultimoencuentro/{equipo}/{usuario}/{examen}', function($request) {
        ...
        if (isset($val01) && isset($val02) && isset($val03)) {
                $sql00  = "SELECT TOP 4
                    ...
                    WHERE (a.EQUIPO_LOCAL_CODIGO = ? OR a.EQUIPO_VISITANTE_CODIGO = ?) AND a.JUEGO_NOMBRE IS NOT NULL AND EXISTS(SELECT * FROM adm.PERCOM WHERE (PERCOMCOC = a.COMPETICION_ID OR PERCOMCOC = a.COMPETICION_PADRE_ID) AND PERCOMPEC = ? AND PERCOMTMC = (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'USUARIOMODULO' AND DOMFICPAR = ?))
