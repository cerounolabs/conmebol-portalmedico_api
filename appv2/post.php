...
/*MODULO NOTIFICACIONES*/
    $app->post('/v2/802/notificacion', function($request) {
        ...
        if (isset($val01) && isset($val03) && isset($val04)) {
            $sql00  = "INSERT INTO [adm].[NOTFIC](                                                    NOTFICEST,  NOTFICORD,                                                                                NOTFICTNC,                                                                                 NOTFICTTC, NOTFICPAC, NOTFICTIT,   NOTFICDES, NOTFICFED, NOTFICFEH, NOTFICOBS, NOTFICAUS, NOTFICAFH, NOTFICAIP) 
            SELECT  (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'NOTIFICACIONESTADO' AND DOMFICPAR = ?),         ?, (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'NOTIFICACIONTIPO' AND DOMFICPAR = ?), (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'EXAMENMEDICOTIPO' AND DOMFICPAR = ?),        ?,          ?,          ?,          ?,        ?,         ?,          ?, GETDATE(), ?)
                            WHERE NOT EXISTS(SELECT * FROM [adm].[NOTFIC] WHERE NOTFICTTC = (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'EXAMENMEDICOTIPO' AND DOMFICPAR = ?) AND NOTFICAFH = GETDATE() AND NOTFICTIT = ? AND NOTFICDES = ?)";
